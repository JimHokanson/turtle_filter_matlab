...
#define STD_DEF mwSize n_data_samples, mwSize b_length, mwSize a_length, double *b, double *a, double *x, double *y, int IIR_option
    
#define STD_Call n_data_samples, b_length, a_length, b, a, x, y, IIR_option
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs == 5){
        //TODO: Check type of 4
        SIMD_option = (int)mxGetScalar(prhs[4]);
    }
...
if (s.HW_AVX2 && s.OS_AVX && SIMD_option == 0){
    fir_avx2_std(STD_Call);
}else if(s.HW_AVX && s.OS_AVX && SIMD_option <= 1){
